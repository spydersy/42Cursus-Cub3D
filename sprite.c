...
void	draw_sprite(int id)
{
	...
	while (++i < size - 1 && (j = -1))
	{
		if (SP[id].xof + i <= 0 || SP[id].xof + i >= DATA.resolution[0])
			continue ;
		if (g_wall_distances[(int)(SP[id].xof + i)] <= SP[id].distance)
			continue ;
		while (++j < size - 1)
		{
			if (SP[id].yof + j <= 0 || SP[id].yof + j >= DATA.resolution[1])
				continue ;
			c = first_color[(int)((64) *
					(64 * j / (int)size) + (64 * i / (int)size))];
			if (c != first_color[0])
				my_mlx_pixel_put(&g_img, i + SP[id].xof, j + SP[id].yof, c);
		}
	}
}
...
void	to_sprite(void)
{
	...
	while (++k < g_nb_sprite)
	{
		angle = atan2f(SP[k].y - g_player.y, SP[k].x - g_player.x);
		while (angle - P.angle > PI)
			angle -= 2 * PI;
		while (angle - P.angle < -PI)
			angle += 2 * PI;
		SP[k].size = (float)((DATA.resolution[0] / SP[k].distance) * 64);
		SP[k].yof = (float)(DATA.resolution[1] / 2 - SP[k].size / 2);
		SP[k].xof = (float)(((deg(angle) - deg(P.angle)) * DATA.resolution[0])
		/ 64 + ((DATA.resolution[0] / 2) - (int)SP[k].size / 2));
		draw_sprite(k);
	}
}
